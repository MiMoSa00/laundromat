...
export default function ServicesPage() {
  ...
  const handleBookService = (serviceId: string) => {
    const bookedService = services.find(service => service.id === serviceId);
    if (bookedService) {
      localStorage.setItem('currentOrder', JSON.stringify({
        id: `ORD-${Date.now()}`,
        status: 'pending',
        items: [{ service: bookedService.name, quantity: 1, price: bookedService.basePrice }],
        totalAmount: bookedService.basePrice,
        ...
        <div className="min-h-screen pb-20 bg-gradient-to-b from-navy to-reddish">
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
            <motion.div
              ...
              className="text-center mb-8 sm:mb-12 mt-12 lg:mt-0"
            >
